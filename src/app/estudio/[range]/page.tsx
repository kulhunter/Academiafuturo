...
import { bancoPreguntas, Question } from '@/lib/preguntas';

interface PageProps {
  params: Promise<{ range: string }>;
}

export default function RangeStudyPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    // Filter questions by ID (since ID corresponds to the sequence in the file)
    const filtered = bancoPreguntas.filter(q => q.id >= start && q.id <= end);
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4">
        ...
        <Link href="/estudio" className="text-blue-600 underline">Volver</Link>
      </div>
    );
  }
